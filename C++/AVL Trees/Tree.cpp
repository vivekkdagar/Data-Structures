...
Node *Tree::RInsert(Node *p, int key) {
    Node *t = nullptr;
    if (not p) {
        t = new Node(nullptr, key, 1, nullptr);         // Starting height from 1 onward
        return t;
    }
...
int Tree::NodeHeight(Node *pNode) {
    int hL, hR;
    hL = pNode and pNode->lChild ? pNode->lChild->height : 0;
    hR = pNode and pNode->rChild ? pNode->rChild->height : 0;

    return std::max(hL, hR) + 1;
}

int Tree::BalanceFactor(Node *pNode) {
    int hL, hR;
    hL = pNode and pNode->lChild ? pNode->lChild->height : 0;
    hR = pNode and pNode->rChild ? pNode->rChild->height : 0;
...
Node *Tree::Remove(Node *p, int key) {
    if (not p)
        return nullptr;
